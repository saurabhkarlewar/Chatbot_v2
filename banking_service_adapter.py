...
class MyLogicAdapter(LogicAdapter):
    ...
    def can_process(self, statement):
        ...
        saving_words = ['savings','saving','salary','sav','sa']
        if any(x in statement.text.lower().split() for x in words) and any(x in statement.text.lower().split() for x in saving_words):
            return True
        else:
            return False
    def process(self, input_statement, additional_response_selection_parameters):
        ...
        print(data)
        balance = data[(data['DATE']==max(data['DATE'])) & (data['Account Type']=='Savings')]['BALANCE AMT'].iloc[0]
        print(balance)
        if balance > 0:
            confidence = 1
        else:
        ...
        response_statement = Statement(text='The current balance in your Savings account is INR {}'.format(balance))
